...
pub fn parse_toml_config(log: &mut Logger) -> Config {
    if !file_exists!(CONFIG_PATH) {
        log.warn(format!("the config file `{CONFIG_PATH}` does not exist, creating it with default config...\n").as_str());
        ...
        match conf_file {
            ...
            Err(msg) => {
                log.err(format!("failed to open config file `{CONFIG_PATH}` for writing because of error: {msg}\n").as_str());
                pause_exit!();
            }
        }
    }
    ...
    if config_contents.trim().is_empty() {
        ...
        match conf_file {
            Ok(mut fd) => match write!(&mut fd, "{}", DEFAULT_CONFIG) {
                Ok(_) => {
                    log.warn(format!("please enter your values into the config file `{CONFIG_PATH}` and restart the program.\n").as_str());
                    pause_exit!();
                }
                Err(msg) => {
                    log.err(format!("failed to open config file `{CONFIG_PATH}` for writing because of error: {msg}\n").as_str());
            ...
            Err(msg) => {
                log.err(format!("failed to open config file `{CONFIG_PATH}` because of error: {msg}\n").as_str());
                pause_exit!();
            }
        }
    }
    ...
    return match toml::from_str(&config_contents) {
        Ok(conf) => conf,
        Err(msg) => {
            log.err(format!("in config file `{CONFIG_PATH}`: failed to parse config because of error: {msg}\n").as_str());
            pause_exit!();
        }
    };
}
